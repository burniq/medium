...
OVERLAY_HOME="$home_dir" cargo run -p medium-cli --bin medium -- \
...
if OVERLAY_HOME="$home_dir" cargo run -p medium-cli --bin medium -- ssh sync \
...
target_pid=$!
printf '' | OVERLAY_HOME="$home_dir" cargo run -p medium-cli --bin medium -- \
  proxy ssh --device node-1 >"$banner_log"
wait "$target_pid"
target_pid=""
grep -q "SSH-2.0-OverlayTest" "$banner_log"

OVERLAY_HOME="$home_dir" cargo run -p medium-cli --bin medium -- ssh sync >/dev/null
backup_file="$(find "$home_dir/.ssh/config.d" -name 'medium.bak-*' -print -quit)"
test -n "$backup_file"

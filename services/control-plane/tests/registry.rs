use overlay_protocol::{EndpointKind, NodeEndpoint, PublishedService, RegisterNodeRequest, ServiceKind};
...
async fn registry_returns_devices_from_registered_nodes() {
    ...
    assert_eq!(catalog.devices[0].name, "Home");
    assert_eq!(
...
async fn registry_resolves_service_route_for_session_open() {
    ...
    let route = store.resolve_service_route("svc_home_ssh").await.unwrap();

    assert_eq!(route.node_id, "node-home");
